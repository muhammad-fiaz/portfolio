...
const Chatbot = () => {
  ...
  const [lastRequestTime, setLastRequestTime] = useState<number>(Date.now()); // Track the last request time
  ...
  const handleSendMessage = async () => {
    ...
    const savedRequestCount = parseInt(getCookie('requestCount') as string) || 0;
    const savedLastRequestTime = parseInt(getCookie('lastRequestTime') as string) || Date.now();
    ...
    if (currentTime - savedLastRequestTime >= 60000) {
      ...
      setCookie('requestCount', '0', { maxAge: 60 }); // Set cookie for 1 minute
    ...
    setLoading(true);
    setRequestCount(savedRequestCount + 1); // Increment request count
    setCookie('requestCount', (savedRequestCount + 1).toString(), { maxAge: 60 }); // Set cookie for 1 minute
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      const initialMessage = { text: 'Hello, how can I assist you today?', sender: 'bot' };
          ...
          {/* Chatbot Header */}
            ...
            <div className="mt-2 flex flex-col items-center justify-center gap-2">
              <Textarea
                ...
                value={message}
                onChange={(e) => handleMessageChange(e as unknown as React.ChangeEvent<HTMLTextAreaElement>)}
              />
              <Button
        ...
        {isRateLimitReached && (
          <div className="fixed top-0 left-0 w-full h-full bg-black/60 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-neutral-800 rounded-xl p-6 shadow-lg text-center">
              <h3 className="font-bold text-lg text-foreground dark:text-white mb-4">Rate Limit Reached! 😬</h3>
