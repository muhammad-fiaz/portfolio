...
const Credits = () => {
  ...
  useEffect(() => {
    const fetchRepoStats = async () => {
      try {
        const response = await fetch(`https://api.github.com/repos/${siteConfig.social.github}/portfolio`);
        if (response.ok) {
  ...
  return (
    <AnimationContainer customClassName="text-center py-4 text-gray-500 text-sm ">
      ...
      <p>
        Built with ❤️ by{" "}
        <Link href={siteConfig.baseUrl} className="text-blue-500 hover:underline">
          {siteConfig.author}
        </Link>
      ...
      <div className="flex justify-center items-center gap-4 mt-2">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 32 32" className="mb-1">
            <path
              fill="currentColor"
